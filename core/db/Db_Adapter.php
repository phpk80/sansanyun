...
abstract class Db_Adapter
{
    ...
    public function table($tableName){
        if(!$tableName) return '';
        return Conf::get('db','prefix').$tableName;
    }
    /**
     * 用于重置成员属性
     */
    ...
    private function check($array, $filter){
        ...
        foreach($array as $key=>$value){
            LogFactory::factory()->log($value,'var');
            $key=strtolower($key);
    ...
    function r_select(){
        ...
        foreach($args as $tab) {
            ...
            if(!empty($tab[3])) {
                ...
                if(!empty($where[$fk])) {
                    $data=Model($tabName)->field($field)->where($where)->select();
                    foreach($data as $row){
                        foreach($one as $read){
                            if($read[$pri]==$row[$fk]){
                                $new[]=$read+$row;
