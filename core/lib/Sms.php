...
class Sms
{
    ...
    public function __construct($sms_sp = '')
    {
        ...
        $val = get_cache($sms_sp);
        $this->sms_sp = $sms_sp;
        $this->sms_config = $val;
        require PLUGIN . '/sms/' . $this->sms_sp . '/' . $this->sms_sp . '.php';
    }
    ...
    {
        $sms = new $this->sms_sp;
        if (!isset($this->sms_config['config'][$tpl_type]) || $this->sms_config['config'][$tpl_type] == '') {
            return array('code' => '', 'err' => '未配置模板');
        }
...
{
    $db = Model('sms_session');
    $db->insert(array('mobile' => $mobile, 'code' => $code, 'sendtime' => time(), 'ip' => getip(), 'tpl_type' => $tpl_type));
}
...
{
    global $db;
    return $db->query("select id ,sendtime from " . $db->table('sms_session') .  " 
                  where mobile='" . $mobile . "' and code='" . $code . "' 
                  and status=0 and tpl_type='" . $tpl_type ."' order by sendtime desc limit 0,1", 'find');
}
...
{
    $db = Model('sms_session');
    return $db->where(array('mobile' => $mobile, 'tpl_type' => $tpl_type))->update(array('status' => 1));
}
...
function get_sms_count($mobile = '', $tpl_type = '', $minute = 2, $is_ip = 0)
{
    ...
    $row = $db->query("select count(*) sms_count from " . $db->table('sms_session') . " where 1 and tpl_type='" . $tpl_type . "' and sendtime>=" . (time() - intval($minute) * 60) . $where, 'total');
