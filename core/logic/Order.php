...
class Order
{
    ...
    function get_cart_amount($is_new = 0)
    {
        ...
        if ($_COOKIE['cnum']==0 || $is_new == 1) {
            ...
            $sql = "select sum(b.num) cnum from " . $db->table('cart') . " a left join " . $db->table('cart_item') . " b on a.id=b.cid where 1 " . $where;
...
//获取收货地址
  static  function get_consignee($id = 0, $uid = 0, $is_default = -1)
    {
        global $db;
        $where = '';
        if ($id != 0) {
        ...
        return $db->query("SELECT m.*,concat(ifnull(p.name,''),ifnull(c.name,''),ifnull(a.name,''),ifnull(d.name,''),address) addr,p.name province_name,c.name city_name,a.name area_name,d.name town_name FROM " . $db->table('member_address') . " m left join " . $db->table('district') . " p on m.province=p.id left join " . $db->table('district') . "  c on m.city=c.id left join " . $db->table('district') . " a on m.area=a.id left join " . $db->table('district') . "  d on m.town=d.id where uid=" . $uid . $where,'select');
    }
...
//计算运费
    static function get_express_fee($uid = 0, $cart = array(), $express_id = 0, $city_id = 0)
    {
    ...
    static function get_max_express_fee()
    {
        $express = get_cache("express");
...
//配送方式
    ...
    function get_express_district($id = 0, $status = -1)
    {
        ...
        return $db->query("select b.code,b.insure,a.* from " . $db->table('express_district') . " a join " . $db->table('express_common') . " b on a.express_id=b.id where 1 " . $where,'select');
    }
...
//生成订单编号
  static  function build_order_sn($is_fullyear = 0)
    {
...
//订单基本信息
 static   function get_order_info($id = 0, $order_sn = '', $uid = 0)
    {
        global $db;
        $where = '';
        if ($id != 0) {
        ...
        $order = $db->query("select o.*, m.uname from " . $db->table('order') . " o left join " . $db->table('member') . " m on o.uid=m.id where 1 " . $where,'find');

        $order['status_name'] = Order::order_status($v['status']);
...
//订单详情
    ...
    static function get_order_count($id = 0, $order_sn = '', $uid = 0, $where)
    {
        global $db;
        ...
        $row = $db->query("select count(distinct o.id) count from " . $db->table('order') . " o join " . $db->table('order_goods') . " og on o.id=og.order_id left join " . $db->table('goods') . " g on g.goods_id=og.goods_id left join " . $db->table('member') . " m on o.uid=m.id where 1 " . $where,'find');

        return intval($row['count']);
    }

//订单列表
  static  function get_order_list($id = 0, $order_sn = '', $uid = 0, $where = '', $start = 0, $num = 10)
    {
        global $db;
        ...
        if ($row) {
            foreach ($row as $k => $v) {
                if (count($order[$v['oid']]) == 0) {
                    ...
                    $order[$v['oid']]['payble_amount'] = format_price($v['payble_amount']);
                    ...
                    $order[$v['oid']]['status_name'] = Order::order_status($v['status']);
                }
                $v['thumb'] = url_to_abs($v['thumb']);
                $v['url'] = $v['code'] . '-g.html';
                $v['spec'] = '';
                if ($v['spec_ids'] != '') {
                    $v['spec'] = Goods::get_spec_val($v['spec_ids'], $v['spec_name']);
                }
                $order[$v['oid']]['goods'][] = $v;
            }
        }
