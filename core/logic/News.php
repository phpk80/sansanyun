...
class News
{
   static function get_spage($id = 0, $c_toid =0) {
        if($id == 0 && $c_toid == 0)
        {
            return false;
        }
        ...
        $row = $db->query("select a.*,b.c_code from ".$db->table('page')." a join ".$db->table('columns')." b on a.c_toid=b.id  where 1 ". $where,'find');
...
//获取文章
    function get_newslist($cids ='',$start=0, $num=12) {
        ...
        $row = $db->fetchall('news', '*', $where, 'c_time desc', $start.','.$num);
        foreach($row as $k=>$v){
            $row[$k]['id']=$v['id'];
            $row[$k]['i']= $k+1;
