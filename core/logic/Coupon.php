...
class Coupon
{
    static function get_couponinfo($id, $is_item = true)
    {
        global $db;
        ...
        if ($is_item) {
            $row['items'] = self::get_coupon_item($id, $row['type']);
            $data = array();
    ...
    /*获取优惠券的品类/商品*/
    static function get_coupon_item($cid, $type)
    {
        global $db;

        if ($type == coupon_type_cat) {
            $row = $db->query("select item_id id,ifnull(name,'全品类') name from " . $db->table('coupon_item') . " i left join " . $db->table('category') . " c on i.item_id=c.id where cid=" . intval($cid),'select');
        } elseif ($type == coupon_type_goods) {
            $row = $db->query("select item_id id,ifnull(name,'全品类') name from " . $db->table('coupon_item') . " i left join " . $db->table('goods') . " c on i.item_id=c.goods_id where cid=" . intval($cid),'select');
        } else {
            return array();
        }
    ...
    /*更新已领取数量*/
    function update_coupon_getNum($id, $num = 1)
    {
        global $db;
        return $db->query("update " . $db->table('coupon') . " set get_num=get_num+" . intval($num) . " where id=" . intval($id));
    }
     ...
     * */
   static function get_coupon_count($where = '', $status_type = 1)
    {
       $db = Model('coupon');
     ...
     * @param int $status_type 状态类型：0不限，1未过期, 2已过期
     * */
   static function get_coupon_list($where = '', $status_type = 1, $startI = 0, $pagenum = null, $uid = 0)
    {
    ...
    /*获取优惠券*/
   static function get_coupon_user($uid = 0, $cid = 0)
    {
        global $db;
        $where = array();
        if ($uid != 0) {
    ...
    /*获取(每天)已领取数量*/
  static  function get_receiver_count($cid, $is_curdate = true)
    {
        $db= Model('coupon_user');
        if ($is_curdate) {
            $where = " and get_time>=" . strtotime(date("Y-m-d"));
        }

        $row = $db->where( "cid=" . intval($cid) . $where)->total();
        return intval($row);
    }
    ...
    function get_mycoupon_sum($uid)
    {
        ...
        $row = $db->query("select count(u.id) count, u.status from " . $db->table('coupon_user') . " u join " . $db->table('coupon') . " c on u.cid=c.id where uid=" . intval($uid) . " group by u.status",'select');
    ...
    /*获取优惠券内的分类或商品id*/
    static function get_coupon_itemids($cid)
    {
        global $db;

        $coupon = self::get_couponinfo($cid, false);
        if (!$coupon) {
            return false;
        }
        $coupon_item = self::get_coupon_item($cid, $coupon['type']);
