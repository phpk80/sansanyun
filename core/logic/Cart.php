...
class Cart
{
    ...
    //获取购物车里的商品
    static function get_cart($status= -1)
    {
        global $db, $uid;
        include CORE.'/common/promotion.php';
        ...
        foreach ($row as $k => $v) {
            ...
            if($v['spec_ids'] !='')
            {
                $row[$k]['spec'] = Goods::get_spec_val($v['spec_ids'], $v['spec_name']);
            }
            else {
                $row[$k]['spec'] = '';
            }
  ...
  static  function get_cart_goods($id = 0, $uid =0)
    {
            ...
            return $db->query("SELECT c.id as cid,c.uid,i.gid,i.pid,i.num,i.spec,i.status from ".$db->table('cart')." c left join ".$db->table('cart_item')." i on c.id=i.cid where 1 ". $where,'select');
...
//删除购物车
    ...
    /*将购物车的所有商品设为不选中*/
 static   function update_cart($id=0, $uid=0)
    {
        global $db;
        $tablepre = $db->pre;
        $where = '';
        ...
        $db->query("update ".$tablepre."cart_item i join ".$tablepre."cart c on c.id=i.cid set i.status=0 where 1 ".$where,'update');
    }

//合并游客购物车里的商品到当前登录用户, 或者合并第三方登陆的到已有账户
   static function merge_cart($uid, $sub_uid=0)
    {
        global $db,$direct,$return_url;
        if($sub_uid !=0)
        {
            $cart_goods_cookie = self::get_cart_goods(0, $sub_uid);
