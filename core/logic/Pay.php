...
class Pay
{
    static function get_payment($pay_code='', $status = 1, $client='0', $no_bal= 1)
    {
...
//增加支付记录
  static  function add_pay_log($pay_sn,$amount, $type, $uid=0, $order_sn='')
    {
        $db = Model('pay_log');

        $db->insert( array('pay_sn'=>$pay_sn,'order_sn'=>$order_sn, 'amount'=>floatval($amount),'type'=>intval($type),'trade_no'=>$trade_no,'trade_buyer'=>$trade_buyer,'trade_msg'=>$trade_msg,'pay_status'=>0,'uid'=>intval($uid), 'addtime'=>time()));
    }
...
//删除未支付记录
   static function del_pay_log($pay_sn)
    {
        $db = Model('pay_log');
        $db->delete( array('pay_sn'=>trim($pay_sn)));
    }

//获取支付记录
   static function get_pay_log($pay_sn='', $trade_no='', $pay_status=-1, $uid=0, $order_sn='')
    {
        global $db;
        $where ='';
        if($pay_sn !='')
        ...
        return $db->query('select p.*,amount as payble_amount,m.uname from '.$db->table('pay_log')." p left join ".$db->table('member')." m on p.uid=m.id  where 1 ".$where,'select');
    }
