...
class User
{
    ...
    {
        $this->uid = $this->get_userid();
    }

    static public  function check_login($is_ajax = 0)
    {
    if(!isset($_SESSION['uid']) || intval($_SESSION['uid'])==0)
    {
        if(!isset($_COOKIE['user']) || trim($_COOKIE['user'])=='')
        {
            ...
            $_SESSION['uid']= ucode($userinfo['uid'], TOKEN) ;
...
function get_openid($uid)
{
    ...
    global $db;
    $row = $db->query("select openid from ". $db->table('member_oauth') ."  where type='wx' and platform='mp' and uid=".intval($uid));
    return $row['openid'];
}

//生成用户名
static function build_username()
{
    $username = "sansan".date('ymd').substr(implode(NULL, array_map('ord', str_split(substr(uniqid(), 7, 13), 1))), 0, 8);
    if(self::check_user_field('uname', $username))
    {
        self::build_username();
    }
    return $username;
...
/**获取会员信息*/
static function     get_user($uid =0)
{
    global $db;

    $ym_uid =  self::get_userid();
    $uid= $uid==0 ?$ym_uid: intval($uid);
    $row = $db->query("select m.*,g.grade_name,g.discount*0.01 discount,g.point_require from ".$db->table('member')." m left join ".$db->table('member_grade')." g on m.grade_id = g.grade_id where id=".$uid,'find');
...
/**获取会员信息*/
static function get_user_by_mobile($mobile='', $uname ='')
{
    global $db;
    if($mobile !='')
    {
...
//增加会员
static function add_user($username, $mobile, $password, $img='', $email='',$birthday=0, $qq='',$sex=0,$realname='',$pids= array(), $commiss_id=0, $grade_id=0)
{
     $db = Model('member');
    $salt= get_salt();
    $grade_id= $grade_id ==0 ? User::get_default_gradeid() : intval($grade_id);
    $result = $db->insert(array('uname'=>trim($username),
        'mobile'=>trim($mobile),'pwd' => encryptStr($password, $salt),
        'salt' =>$salt,'status'=>normal,'addtime'=>time(),
        'logintime'=>time(),'ip'=>getip(),'img'=>trim($img),
        'email'=>trim($email),'birthday'=>trim($birthday),
        'qq'=>trim($qq),'sex'=>intval($sex),'realname'=>trim($realname),
        'pid1'=>$pids['pid1'],'pid2'=>$pids['pid2'],'pid3'=>$pids['pid3'],
        'commiss_id'=>intval($commiss_id), 'grade_id'=>$grade_id));
...
//更新用户信息
static function update_userinfo($uid, $userinfo=array())
{
    $db = \Model('member');
    $db->where(array('id'=>$uid))->update($userinfo);
}
...
static function check_user_field($field, $val)
{
    $db = Model('member');
    $user = $db->where(array($field=>$val))->select($field);
...
//获取第三方登陆方式
static function get_oauth($status=1, $code='')
{
    global $db;
...
//拉取第三方登录头像
static function get_oauthimg($imgurl)
{
...
//登录后信息处理
static function set_login_session($uid, $uname='', $autologin=1, $is_merge_cart=true)
{


    global $db,$sansan_client;
    ...
    $data = array('uid'=>jcode($uid, TOKEN),'uname'=>$uname);
    $login_time = intval($autologin)==1 ? time() + 15552000 : "0";

    set_cookie('user', json_encode($data), $login_time);

    self::update_userinfo($uid, array('logintime'=>time(), 'ip'=>getip()), array('id'=>$uid));

    if($is_merge_cart)
    {
        Cart::merge_cart($uid); //合并游客购物车
    }

    System::add_login_log($uid, role_user, login_success);//登录日志

    //保存登录状态
    User::add_member_token($uid, time()+7776000);//3个月过期
}

static function get_sid()
{
    return urlencode(endecrypt(session_id(), 'encode', TOKEN));
...
//清空某用户
static  function del_session($sid)
{
     $db = Model('member_token');

    $db->where(array('sid'=>trim($sid)))->delete();
...
static function add_member_token($uid, $expire)
{
     $db = Model('member_token');
    ...
    if(empty($ym_sid))
    {
        $db->insert(array('sid'=>session_id(), 'uid'=>$uid,'expire'=>$expire,'ip'=>getip()));
    }
    else {
        $row = $db->total(array('sid'=>trim($ym_sid)));
        if($row && $row>0)
        {
            $db->where(array('sid'=>$ym_sid))->update(array('expire'=>$expire,'ip'=>getip()));
        }
        else {
            $db->insert( array('sid'=>$ym_sid, 'uid'=>$uid,'expire'=>$expire,'ip'=>getip()));
        }
    }
}
...
//检测用户名
static function check_uname($username, $is_exist=false, $uid=0)
{
    ...
    if($is_exist)
    {
        $row = self::check_username($username, $uid);
        if($row)
        {
            return "该用户名已注册";
        }
...
//查询用户名
static function check_username($val, $uid = 0)
{
     $db = Model('member');
    $where = $uid == 0 ? '' : " and id<>".intval($uid);
    return $db->where("(uname='".$val."' or mobile='".$val."')".$where)->find('id,mobile');
}
...
function get_fav($uid, $goods_id=0,$spec='',$start=0 , $pagenum=10)
{
    ...
    return $db->query("select g.thumb,g.code,g.name,ifnull(s.`values`,'') spec_name,ifnull(s.price,g.price) price, f.* from ".$db->table('member_fav')." f join ".$db->table('goods')." g on f.goods_id=g.goods_id left join ".$db->table('goods_spec')." s on f.spec=s.values and f.goods_id=s.goods_id where f.uid=".$uid.$where." limit ".$start.",".$pagenum,'select');
}
...
function get_comment_starcount($goods_id, $uid)
{
    ...
    $row =$db->query("select (case star when 4 then 'good' when 5 then 'good' when 2 then 'mid' when 3 then 'mid' else 'bad' end) starlevel,count(*) count from ".$db->table('comment')." where type=0 and item_id=".intval($goods_id).$where." group by starlevel order by addtime",'select');
...
function get_comment_list($goods_id, $star_level='', $type=0, $start =0, $num =10, $uid=0)
{
    ...
    $row = $db->query("select c.*,m.uname,m.img uimg,grade_name from ".$db->table('comment')." c left join ".$db->table('member')." m on c.uid=m.id left join ".$db->table('member_grade')." mg on m.grade_id=mg.grade_id where c.type=".intval($type)." and c.item_id=".intval($goods_id).$where." order by addtime desc limit ".$start.",".$num,'select');

    foreach ($row as $k => $v) {
        $reply_count= $db->query("select count(*) count from ".$db->table('comment_reply')." where status=1 and cid=".$v['id']." and role_type=".role_user,'find');
        $row[$k]['reply_count'] = intval($reply_count['count']); //回复数量
        $row[$k]['admin_reply'] = self::get_comment_reply(0,$v['id'],$v['id'], 0, role_admin); //管理员回复
 ...
 *@param $pid_type 回复的对象：0评论 1回复
 * */
static function get_comment_reply($id=0, $cid=0, $pid=0, $pid_type=0, $role_type = null,$uid=0, $reply_uid=0)
{
    global $db;
    $where ='';
    if($id !=0)
...
//获取默认等级ID
static function get_default_gradeid()
{
    $db = \Model('member_grade');
    $row = $db->where(array('is_default'=>1))->field('grade_id')->find();
...
function get_comment($order_sn='', $uid=0)
{
    ...
    return $db->query("select c.*,g.name from ".$db->table('comment')." c join ".$db->table('goods')." g on c.goods_id=g.goods_id where ".$where,'select');
}
...
/*获取等级名称*/
static function get_grade_name($ids, $grade = null)
{
    if(empty($ids))
    {
        return '所有';
