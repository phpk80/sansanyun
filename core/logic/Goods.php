...
class Goods
{
    ...
    {
        self::$model['goods'] = Model('goods');
        self::$model['brand'] = Model('brand');
        self::$model['goods_cat'] = Model('goods_cat');
     }

 public static  function get_childIDs($cat_id = 0, $res_arr=false, $level= 0)
    {
        ...
        $ids = self::get_catIds($ym_cat, $cat_id, $level);
...
//获取子分类的id
 static   function get_catIds($rows, $pid = 0, $level = 0, $id_name = 'id', $pid_name = 'pid')
    {
        global $arr_ids ;
        ...
        foreach($rows as $key => $value)
        {
            if($value[$pid_name] == $pid)
            {
                ...
                self::get_catIds($rows, $value[$id_name], $level);
            }
        }
        return $arr_ids;
    }
...
//获取下一级子分类
public static   function get_children($cat_id = 0)
    {
        global $ym_cats;
...
//获取子分类
     ...
     * */
   static function get_goods_list($where='',$field ='', $sort = '', $startI=0, $num=10, $is_up=1, $is_formated=0,$wordreplace="")
    {
        global $db,$user;
        $uid = $user->get_userid();
        ...
        $row = $db->query($sql,'select');
        foreach ($row as $k => $v) {
            if ($wordreplace){
                for ($z=0;$z<count($wordreplace);$z++){
                    $v['name']=str_replace($wordreplace[$z],'<b style="color:red;">'.$wordreplace[$z].'</b>',$v['name']);
                }
            }
            $row[$k]['name']=$v['name'];
            ...
            $discount_price = get_discount_price($v['goods_id'], $uid, $v['price']);//获取活动价格
            $tmp_price = self::get_min_max_price($v['goods_id']);
            ...
            $row[$k]['goods_price'] = $tmp_price['min_price'] && $tmp_price['min_price']!==0 ? format_price(get_discount_price($v['goods_id'], $uid, $tmp_price['min_price'])) : format_price($discount_price);
        }

        return $row;
    }

    static function get_min_max_price($goods_id)
    {
        global $db;
   ...
   static function get_goods($goods_id=0)
    {
    ...
    /*扩展分类商品*/
public static   function get_extend_goods($cat_id)
    {
      //  global $db;
       // $row = $db->fetchall('goods_cat', 'goods_id', array('cat_id'=>intval($cat_id)));
        new self();
        $row = self::$model['goods_cat']->where(array('cat_id'=>intval($cat_id)))->select();
        $data = array();
        if(!$row || count($row)==0)
        {
            return create_in('');
    ...
    /*获取商品库存*/
 static   function get_goods_num($goods_id=0, $spec='')
    {
        global $db;
        ...
        if($spec !='')
        {
            ...
            return $db->query("SELECT g.status goods_status,g.uptime,ifnull(s.number, g.number) number,ifnull(s.values,'') spec from ".$db->table('goods'). " g left join ".$db->table('goods_spec'). " s on g.goods_id=s.goods_id where g.goods_id=".intval($goods_id). " and (s.`values`='".$spec."' or s.`values`='".$spec2."')",'find');
        }
        else {
            return $db->query("SELECT g.*,g.status goods_status,g.uptime,g.number,'' spec from ".$db->table('goods'). " g  where g.goods_id=".intval($goods_id),'find');
        }
    }
...
//获取商品某规格的信息
    ...
    /**更新商品*/
   static function update_goods($goods_id=0, $goods = array())
    {
        $db = Model('goods');
        $db->where(array('goods_id'=>intval($goods_id)))->update($goods);
    }

//获取某分类信息
public static  function get_catInfo($cid)
    {
        global $ym_cats;
        $list = array();
        if(is_array($cid))
...
public static   function get_attrs($type_id=array() , $id =0)
    {
...
//获取指定分类商品
    static function get_diy_goods($list=array(), $cid = 0)
    {
    ...
    /**获取某商品的规格*/
   static function get_goods_spec($gid)
    {
        global $db;
...
//获取某商品的规格
  static  function get_spec_val($spec_ids, $spec_name)
    {
...
//获取某商品的参数
  static  function get_attr_val($goods_id=0)
    {
        if(intval($goods_id) == 0)
        {
            return false;
        ...
        global $db;
        $row= $db->query("select a.id,a.name,a.uitype,ga.values from ".$db->table('goods_attr' )." ga join ".$db->table('attribute' )." a on ga.attr_ids=a.id where goods_id=".intval($goods_id)." order by `sort`",'select');
    ...
    /*获取最近浏览商品*/
    static function get_history($num)
    {
        ...
        if(count($ids)>0)
        {
            $row = self::get_goods_list("and g.goods_id ".create_in($ids),'', '', 0, $num,1,1);
        }
    ...
    {
        global $db;
        return $db->query("SELECT distinct m.id,m.uname,m.img FROM ".$db->table('member_fav')." f join ".$db->table('member')." m on f.uid=m.id where goods_id=".intval($goods_id),'select');
    }

//上一个商品
    static function get_prev_goods($goods_id)
    {
        global $db;
        $row = $db->query("select goods_id,name,code from ".$db->table('goods')." where goods_id<".intval($goods_id)." order by goods_id desc limit 1",'find');
        $row['url']= get_url($row['code']);
        return $row;
    }

//下一个商品
    static function get_next_goods($goods_id)
    {
        global $db;
        $row = $db->query("select goods_id,name,code from ".$db->table('goods')." where goods_id>".intval($goods_id)." order by goods_id asc limit 1",'find');
...
//获取品牌
   public static function get_brand($id=0, $name='', $recommend=0 , $startI=0, $num=null)
    {
        global $db;
        dump(self::$model);
        exit();
        ...
        return self::$model->where('where status =1'.$where)->select();
...
//获取分类关联的品牌
   public static function get_cat_brand($cid)
    {
        $obj = new self();
        return self::$model['brand']->field('id,name,logo,url')->where('status=1 and find_in_set('.intval($cid).', cat_ids)')->select();
        //global $db;

       // return $db->query("select id,name,logo,url from ".$db->table('brand')." where status=1 and find_in_set(".intval($cid).", cat_ids) ",'select');
    }
...
//会员价
    ...
    {
        global $db;
        return $db->query("select mp.*,mg.grade_name from ".$db->table('member_price')." mp join ".$db->table('member_grade')." mg on mp.grade_id=mg.grade_id where goods_id=".intval($goods_id),'select');
    }

//某会员的会员等级折扣
  static  function get_userGradeDiscount($uid)
    {
        global $db;
   ...
   static function get_grade()
    ...
    function get_catids_by_gids($gids)
    {
        ...
        $row = $db->query("select cat_id from ".$db->table('goods')." where goods_id in(".trim($gids).")",'select');
        if($row)
        {
            foreach($row as $k => $v) {
