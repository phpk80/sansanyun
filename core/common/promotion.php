...
function get_discount($where='',$startI, $pagenum, $is_goods=0)
{
	...
	$row = $db->query("SELECT a.*  FROM ".$db->table('sp_discount')." a where 1 ".$where." order by addtime desc ".$limit,'select');
	foreach ($row as $k => $v) {
		...
		if($is_goods ==1)
		{
			$goods = \core\logic\Goods::get_goods_list('and goods_id in('.$v['goods_ids'].')','goods_id,code,g.name,subname,cat_id,brand_id,g.img,thumb,imgs,number,virtualnum,price,marketprice,g.addtime', '', 0, null);
			$row[$k]['goods'] = $goods;
		}
	}
	return $row;	
...
function get_discount_price($goods_id, $uid=0, $price, $user_discount=0)
{
	...
	$row = $db->query("select type,val from " .$db->table('sp_discount')." WHERE status=1 and start_time<=" .time(). " and end_time>=" .time(). " and find_in_set(".intval($goods_id).", goods_ids) " . $where." order  by addtime desc",'find');
	...
	if($row && count($row)>0)
	{
		...
		if($user_discount ==0)//无折扣或未登录  || $uid ==0
		{
			if($uid==0)
			{
				...
				$user = \core\logic\User::get_user($uid);
